...
function _renderResults(panel, query, typeFilter = 'all') {
    ...
    const filtered = allRestraints.filter(r => {
        ...
        const type = classifySupport(name, keywords);
    ...
    resultsContainer.innerHTML = `
        ...
        <div style="display:flex; flex-direction:column; gap:8px;">
            ${filtered.map(r => {
                ...
                return `
                <div class="restraint-card" style="border:1px solid var(--color-border); border-radius:4px; padding:8px; background:#fafbfc; font-size:12px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                        <strong style="color:var(--color-primary-dark);">${r.name || 'Unnamed'}</strong>
                        <span style="font-size:10px; background:#e1e4e8; padding:2px 4px; border-radius:2px;">${type}</span>
                    </div>
                    <div style="color:#666; margin-bottom:6px;">Node: ${r.node}</div>
